...
export class SolanaEventListener {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!this.config.heliusApiKey || this.config.heliusApiKey === 'your-api-key') {
  ...
  private handleMessage(event: MessageEvent): void {
    ...
    try {
      ...
      if (data.id && typeof data.id === 'string' && data.id.startsWith('health_')) {
    ...
    try {
      switch (eventName) {
        case 'battleRoomCancelled':
          this.eventHandlers.battleRoomCancelled?.(eventData as BattleRoomCancelled, signature)
          break
        case 'warriorReleased':
          this.eventHandlers.warriorReleased?.(eventData as WarriorReleased, signature)
          break
        case 'delegationEvent':
          this.eventHandlers.delegationEvent?.(eventData as DelegationEvent, signature)
          break
        case 'joinEvent':
        ...
        case 'warriorCreatedEvent':
          this.eventHandlers.warriorCreatedEvent?.(eventData as WarriorCreatedEvent, signature)
          break
        default:
  ...
  private attemptReconnect(): void {
    ...
    if (this.status.reconnectAttempts >= this.config.maxReconnectAttempts) {
      this.setError(`Max reconnection attempts (${this.config.maxReconnectAttempts}) reached`)
  ...
  private getHttpEndpoint(): string {
    const baseUrl =
      this.config.network === 'mainnet' ? 'https://mainnet.helius-rpc.com' : 'https://devnet.helius-rpc.com'
    return `${baseUrl}/?api-key=${this.config.heliusApiKey}`
  }

  private getWebSocketEndpoint(): string {
    const baseUrl = this.config.network === 'mainnet' ? 'wss://mainnet.helius-rpc.com' : 'wss://devnet.helius-rpc.com'
    return `${baseUrl}/?api-key=${this.config.heliusApiKey}`
  }
